...
class TVHeadend(object):
    ...
    def __init__(
        ...
        _LOGGER.debug(
            "pyTVHeadend %s initializing new server at: %s", __version__, host
        )
    ...
    async def fetch_subscription_list(self, force=False):
        ...
        slist = await self.api_get(
            self.root_url + SUBSCRIPTIONS_URL,
        ...
        for index, obj in enumerate(self._ext_list):
            log_str = log_str + "Pos: {} - Name: {}, ".format(
                index, obj.channel_name
            )
    ...
    def update_stream_list(self, streams, force):
        ...
        for channel in streams:
            ...
            if stream_name not in self._streams:
                _LOGGER.debug(
                    "New stream: %s. Adding to stream list.", stream_name
                )
        ...
        try:
            for stream, index in tmp_strm.items():
                ...
                if stream not in active_streams:
                    ...
                    _LOGGER.debug(
                        "Old stream: %s. Removing from stream dict.", stream
                    )
